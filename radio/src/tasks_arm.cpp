...
void mixerTask(void * pdata)
{
  ...
  while(1) {
    ...
    if (!s_pulses_paused) {
      ...
      DEBUG_TIMER_START(debugTimerMixer1);
      CoEnterMutexSection(mixerMutex);
      DEBUG_TIMER_START(debugTimerMixer2);
      doMixerCalculations();
      DEBUG_TIMER_STOP(debugTimerMixer2);
      CoLeaveMutexSection(mixerMutex);
      DEBUG_TIMER_STOP(debugTimerMixer1);
...
void scheduleNextMixerCalculation(uint8_t module, uint16_t delay)
{
  ...
  // for now assume mixes calculation takes 2 ms.
